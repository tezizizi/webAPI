...
from apiApp.models import ClickData,PvData
import csv
...
def upload_clickdata(filename):
    with open(filename) as f:
         #        #form_data = TextIOWrapper(request.FILES['csv'].file, encoding='utf-8')
         csv_file = csv.reader(f)
         header = next(csv_file)
         for line in csv_file:
             recommend_relation = ClickData(line[0],line[1],line[2],line[3])
             recommend_relation.save()

def upload_pvdata(filename):
    with open(filename) as f:
    #        #form_data = TextIOWrapper(request.FILES['csv'].file, encoding='utf-8')
        csv_file = csv.reader(f)
        header = next(csv_file)
        for line in csv_file:
            recommend_relation = PvData(line[0],line[1],line[2],line[3])
            recommend_relation.save()
#

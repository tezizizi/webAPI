...
class ArticleViewSet(viewsets.ModelViewSet):#Model:編集可、ReadOnly:参照のみ
    ...
    def get(self, request):
        a = ["hello"]
        return Response(a)
def click_counter(id):
    data = ClickData.objects.get(article_id = id)
    data.click_count += 1
    data.update_date = timezone.now()
    data.save()

def click_count(request):
    path = request.GET.get('cu')
    queryset = CrawledArticle.objects.all()
    if path is not None:
        queryset = queryset.filter(article_url = str(path))

    originalArticle_id = str(queryset[0].article_id)
    click_counter(id = int(originalArticle_id))
